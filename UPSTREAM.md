...
- 用 Biomedical ML 的四章寫作骨架取代示範正文，移除示範作者與示範書目作為預設內容；章節安排依使用者指定參考版本整併，見 docs/THESIS_STRUCTURE.md。
- 新增寫作指南、Thesis Lab 協作規則及可追溯的官方格式檢查文件。
- 移除 NCS 專屬 class、submodule 與未使用的範例；符號與縮寫頁重新提供空白寫作骨架。完整上游歷史仍保存在 Git。舊版 ntuthesis.cls 原樣保留作為來源參照，目前不載入。
